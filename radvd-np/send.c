...
send_ra(int sock, struct Interface *iface, struct in6_addr *dest)
{
	...
	*/	
	...
	/*
	* Add NP++ mapping info to RA if EnNp is on in config file
	*/
	if(iface->EnNp)
	{
		npma.type = 62;
		...
		npma.maps = malloc(sizeof(struct np_map)) ;

		/*
		* NP++ : Read mapping info from config into npma
		*/
		printf("-+-+-+-+-+-+-+-+- RA -+-+-+-+-+-+-+-+-\n") ;
		while(npmc)
		{
			...
			if(npmc->adv)
			{
				...
				npma.maps[npma.no_maps].mapping = npmc->mapping;
				npma.maps[npma.no_maps].prio = npmc->prio;

				printf("Advertising Mapping id %d  --  Prio == %d\n", npmc->mapping, npmc->prio) ;
		...
		/*
		* NP++ : Set the ICMPv6 option length now the number of mappings is known
		*/
		int j = 0;
		...
		*/
		...
		/*
		* NP++ : XXX : Set this length properly, can do this from (npma.length * 8) - ((npma.no_maps * 2) + (sizeof(npma) - sizeof(npma.maps)))
		*/
