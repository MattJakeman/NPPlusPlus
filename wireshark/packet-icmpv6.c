...
static gint ett_icmpv6_np_map = -1; /* NP++ */
static gint ett_icmpv6_np_map_fin = -1; /* NP++ */
...
/*29-252  Unassigned */
	{ ND_OPT_NP_NEGOTIATION,            "NP++ Mapping Negotiation"},
	{ ND_OPT_NP_FINALISATION,           "NP++ Mapping Finalisation"},
...
again:
    ...
    case FMIP6_OPT_NEIGHBOR_ADV_ACK:
		...
		/*
		* NP++ : Negotiation ICMPv6 Option
		*/
		case ND_OPT_NP_NEGOTIATION:
		{
//			struct nd_opt_route_info ribuf, *ri;
			struct nd_opt_np_map_adv nd_opt_np_map_adv, *ni;
			proto_item *soi;
		    proto_tree *mapping_tree;
			int i, mapoff = offset + 3, mapping_type, map_len = 2;

			ni = &nd_opt_np_map_adv;
			tvb_memcpy(tvb, (guint8 *)ni, offset, sizeof *ni);
		    proto_tree_add_text(icmp6opt_tree, tvb,
						        offset + offsetof(struct nd_opt_np_map_adv, nd_opt_npm_no_maps),
						        1, "Number Of Mappings In Packet: %u", ni->nd_opt_npm_no_maps);
			for(i = 0 ; i < ni->nd_opt_npm_no_maps ; i++)
			{
		        soi = proto_tree_add_text(icmp6opt_tree, tvb, mapoff, map_len, "NP++ Mapping Advertisement %d", i+1);
		        mapping_tree = proto_item_add_subtree(soi, ett_icmpv6_np_map);
				mapping_type = tvb_get_guint8(tvb, mapoff);
				switch (mapping_type)
				{
					case 0:
					{
			            proto_tree_add_text(mapping_tree, tvb, mapoff, 1, "Type: Default (0)");
			            proto_tree_add_text(mapping_tree, tvb, mapoff + 1, 1, "Priority: %u", tvb_get_guint8(tvb, mapoff + 1));
						break;
					}
					case 1:
					{
			            proto_tree_add_text(mapping_tree, tvb, mapoff, 1, "Type: Unsupported (1)");
			            proto_tree_add_text(mapping_tree, tvb, mapoff + 1, 1, "Priority: %u", tvb_get_guint8(tvb, mapoff + 1));
						break;
					}
					case 2:
					{
			            proto_tree_add_text(mapping_tree, tvb, mapoff, 1, "Type: Header Compression (2)");
			            proto_tree_add_text(mapping_tree, tvb, mapoff + 1, 1, "Priority: %u", tvb_get_guint8(tvb, mapoff + 1));
						break;
					}
					case 3:
					{
			            proto_tree_add_text(mapping_tree, tvb, mapoff, 1, "Type: Address Translation (3)");
			            proto_tree_add_text(mapping_tree, tvb, mapoff + 1, 1, "Priority: %u", tvb_get_guint8(tvb, mapoff + 1));
						break;
					}
					default:
					{
			            proto_tree_add_text(mapping_tree, tvb, mapoff, 1, "Type: Unknown (%u)", tvb_get_guint8(tvb, mapoff));
			            proto_tree_add_text(mapping_tree, tvb, mapoff + 1, 1, "Priority: %u", tvb_get_guint8(tvb, mapoff + 1));
						break;
					}
				}
			...
			break ;
                ...
                case ND_OPT_NP_FINALISATION:
		{
		  struct nd_opt_np_fin nd_opt_np_fin, *ni;
		  proto_item *soi;
		  
		  ni = &nd_opt_np_fin;
		  tvb_memcpy(tvb, (guint8 *)ni, offset, sizeof *ni);
		  proto_tree_add_text(icmp6opt_tree, tvb,
						        offset + offsetof(struct nd_opt_np_fin, nd_opt_npf_mapping),
						        1, "Finalised Mapping: %u", ni->nd_opt_npf_mapping);
